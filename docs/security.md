...
- Treat formula source as untrusted text. Render it only with the pinned local KaTeX package and local `mhchem` extension using `trust: false`, strict errors, and no user-defined trust callback. Never enable commands that create links, HTML, styles, external resources, or executable DOM, and never persist generated KaTeX HTML or MathML.
- In static history and public views, intercept Vditor math placeholders after sanitized Markdown conversion and before Vditor can load its CDN renderer. Render preserved formula source through TeamMD's local bounded adapter; invalid or oversized formulas must remain local errors and must not remove surrounding content.
...
- KaTeX formula source: 64 KiB UTF-8 per inline or display formula
