...
History lists at most 200 revisions newest-first in the current release. Each item includes revision ID, ordinal, timestamp, author ID/email, UTF-8 byte size, optional save message, and optional restore lineage. Historical content is returned only by the individual revision endpoint and only to owners/editors. The individual response also includes `blackboards`, an ordered bounded array of immutable snapshots with stable ID, name, frozen Markdown background and hash, and validated vector strokes; older revisions return an empty array. A restore returns the same checkpoint response shape as collaborative Save, resets the Yjs room generation, and forces connected clients to rebuild from the new authoritative head.

Collaboration ticket creation requires the authenticated session, CSRF token, allowed origin, and an editor protocol identifier. During migration, supported protocols are `legacy-text-v1`, `milkdown-xml-v1`, and `milkdown-blackboards-v1`. The blackboard protocol preserves the Milkdown `XmlFragment` and adds one validated document-scoped blackboard collection root. The response contains a random one-time token, WebSocket URL, document ID, current permission, negotiated state format, and expiry. Tickets expire after one minute, are bound to one document, session, and negotiated format, and are consumed atomically during WebSocket authentication. A client incompatible with the room format receives a stable upgrade-required error and is never allowed to create a second writable Yjs root. The raw ticket is never stored or logged.

Collaborative checkpoint responses and room checkpoint/restore events include both `contentHash` for canonical main Markdown and `blackboardHash` for the canonical complete blackboard collection. The browser uses both hashes when deciding whether the captured state is Saved; changes made to either layer after capture remain dirty.
