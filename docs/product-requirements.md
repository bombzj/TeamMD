...
- **REQ-BOARD-004:** Every blackboard renders its copied Markdown on a stable logical sheet and owns an independent transparent vector drawing layer above it. Zooming, panning, resizing, switching blackboards, and device-pixel-ratio changes do not alter the copied background or saved stroke geometry. Sheet height is derived from intrinsic Markdown content without resize feedback, and the browser bounds canvas backing dimensions and total pixels independently from logical height.
