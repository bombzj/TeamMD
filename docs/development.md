...
- Keep KaTeX rendering local and derived from canonical `$...$` and `$$...$$` source. Browser and API codecs must install matching math schemas; use `trust: false`, do not add user-defined macros or trust callbacks without security review, and never load Vditor's CDN math renderer or persist generated KaTeX DOM.
