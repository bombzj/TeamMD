...
The `MILKDOWN_BLACKBOARDS_V1` live-state format retains `Y.XmlFragment('milkdown')` and adds one `Y.Map('blackboards')` keyed by stable UUID. Each nested board contains bounded name/order metadata, an immutable canonical Markdown background copy plus hash, and a Yjs map of independently addressable validated vector strokes. A new board background must equal the authoritative room Markdown at creation; later updates may change metadata and strokes but cannot replace the background. This lets concurrent strokes on different boards converge without whole-collection last-write-wins replacement.

The browser measures an intrinsic Markdown-content wrapper rather than the height-stretched sheet, preventing resize-observer feedback. Canvas backing storage is resized only when logical sheet height changes, caps device-pixel ratio at 2, and stays below both 16,384 pixels per backing dimension and 8,388,608 total backing pixels; very tall logical sheets trade raster sharpness for bounded native memory without changing vector geometry. Pointer previews are animation-frame-coalesced and distance-sampled, canvas storage is released on unmount, dirty-state comparison walks canonical snapshots without allocating aggregate JSON strings, and local Yjs Undo/Redo retention is cleared at a bounded 100-operation interval.

Collaboration shutdown first flushes document state, then explicitly releases upgraded WebSocket sockets and their heartbeat timers. The shutdown is idempotent, has a bounded grace period so an unresponsive transport or persistence hook cannot hold a deployment indefinitely, and still disconnects the database when either listener reports a close failure.

Explicit Save reads canonical Markdown and the complete blackboard collection from the same room while holding the checkpoint serialization boundary. The revision transaction creates `DocumentRevision` plus zero or more `DocumentRevisionBlackboard` rows and advances both head pointers atomically. Restore rebuilds both Yjs roots, increments the room generation, and disconnects stale clients. History renders each board's frozen Markdown through the sanitized static renderer and draws only validated application-owned vector primitives; anonymous public views continue to exclude blackboards.
...
Inline `$...$` and display `$$...$$` formulas use Crepe's pinned `remark-math` schema and local KaTeX renderer. Inline formulas are one `math_inline` ProseMirror node and display formulas reuse the canonical code-block node with `LaTeX` language; both serialize back to portable dollar-delimited Markdown. The API codec installs the matching parser, node schema, and display-math serializer before reading or checkpointing the authoritative Yjs room. KaTeX HTML and MathML exist only as disposable browser DOM and never enter Yjs, revisions, APIs, or persistence.

Static history and public rendering intercept Vditor's sanitized `.language-math` placeholders before Vditor can invoke its CDN-based renderer. TeamMD then renders the preserved text with the pinned local KaTeX package and local `mhchem` extension using strict errors, `trust: false`, and bounded source size. Display formulas keep readable intrinsic width inside a preview-local horizontal scroller.

Mermaid editor previews use Crepe's existing fenced-code preview hook. The toolbar and add-block menu expose Mermaid as a first-class Diagram action, but that action still inserts the canonical `code_block` with exact lowercase `mermaid` language and starter source. The `code_block` and its CodeMirror source remain mounted and authoritative; a lazy, editor-scoped renderer derives sanitized SVG and returns it to Crepe's disposable preview panel. Rendering changes neither the Milkdown schema nor the Yjs state format. The renderer serializes work, ignores results after editor teardown, applies source-size, line-count, and time limits, and removes scripts, links, HTML integration points, event/style attributes, and generated style elements before insertion. Trusted application CSS supplies the discarded presentation. A disposable clone is measured under that final CSS so the returned SVG receives a fitted viewBox without first entering a hidden measurement tree. Compact diagrams use intrinsic sizing and responsive downscaling; wide timeline diagrams preserve readable intrinsic scale inside the preview's horizontal scroller. Post-sanitization repair may preserve only same-document SVG references whose prefixed target IDs exist in the sanitized tree; external, malformed, and broken references are removed.

Supported `flowchart TB|TD|BT|RL|LR` source is reparsed into transient visual controls attached to that existing preview. Editors can add, rename, or delete rectangular nodes; add or delete unlabeled directed edges; and change direction. Each action reads the current owning ProseMirror code block at dispatch time, rejects stale, read-only, wrong-language, or unsupported state, and replaces only that block's text in one normal transaction. The established ProseMirror/Yjs history, collaboration, checkpoint, and source-mode paths therefore remain authoritative. Unsupported Mermaid families, subgraphs, edge labels, alternate shapes, duplicate edges, and ambiguous declarations remain preview/source-only and are never normalized. Permission changes remove or reconstruct controls from current source; no graph model is stored in DOM, React, Yjs, revisions, APIs, or persistence.

Static history and public rendering use the same bounded renderer through a separate read-only adapter. Vditor first converts and sanitizes Markdown in a detached host. Its post-conversion transform renames only exact `language-mermaid` code classes before Vditor can invoke its bundled Mermaid renderer, preserving the code text while disabling that unbounded path. After Vditor finishes, the adapter replaces those marked code blocks with disposable preview containers, derives SVG through TeamMD's renderer, and mounts only the result for the currently selected revision. Cleanup destroys pending diagram work and stale Vditor completions never enter the live document. No static rendering code imports Milkdown, ProseMirror, Yjs, or a save API.

Browser configuration is parsed through the explicit `@teammd/config/browser` export. `VITE_MERMAID_RENDERING_ENABLED=false` suppresses only derived editor, history, and public rendering and visual controls. Mermaid fences remain visible/editable source and retain the same collaboration and persistence behavior. Because Vite embeds public configuration at build time, changing this flag requires rebuilding and redeploying the web artifact; it never requires a data migration or source rewrite.
